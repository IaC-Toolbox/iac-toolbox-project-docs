...
**Finding your service region** — look at your PagerDuty URL. If it looks like `https://<company>.eu.pagerduty.com` you are in `eu`. If it is `https://<company>.pagerduty.com` you are in `us`. The region matters for the Terraform provider - using the wrong one causes authentication errors that are not obviously region-related.
...
When a alert is critical enough that it has to flow into PagerDuty, it goes through [Events API v2 API](https://developer.pagerduty.com/docs/events-api-v2-overview) to create incidents. But first we need to make generate an integration key. Conveniently it can be done using terraform. Let's create Pagerduty Terraform Module:
...
The above code outputs the integration key. Note the `var.service` is a variable we will define soon for our service. The service is needed, because incidents are assigned to services. When an alert fires, users who are on-call get notified according to the service's escalation policy. An escalation policy answers the question: *who gets alerted, and what happens if they don't respond?* It defines the sequence of users or teams to notify and the delay between escalation steps. On the free plan, PagerDuty allows one escalation policy — we reference the default one using a `data` source rather than creating a new one.
...
The integration key is the binding pin. It is generated by `pagerduty_service_integration`, which is bound to `pagerduty_service.infra`. Every event Grafana pushes using that key lands on that specific service, which then applies its escalation policy to decide who gets paged.
...
resource "pagerduty_service_integration" "grafana" {
  name    = "Grafana"
  service = pagerduty_service.infra.id          # ← bound to the service above
  type    = "events_api_v2_inbound_integration"
}
