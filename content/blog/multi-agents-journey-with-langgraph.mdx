...
Our agent ran synchronously with a 120-second timeout. In practice: P50 response was around 10 seconds, P99 was around 45 seconds. Log searches took 30–90 seconds. Past incident recall took 45–60 seconds. A single hypothesis — formulate it, search for evidence, evaluate — chained these together and could take several minutes. Sequential hypothesis testing multiplied that. A moderately complex incident with three or four candidate root causes could take 10+ minutes to diagnose, all of it blocking, none of it visible to the user until it was done.

### Confirmation bias baked in

The agent's diagnostic approach was essentially abductive reasoning — infer the most plausible cause from available symptoms. When it found database timeouts and slow queries, it concluded "database performance issue" because that fit the evidence it had. It never checked the alternative: that a cache failure 30 seconds earlier had caused the database load, and the database was a symptom rather than a cause. Sequential diagnosis encourages this. Once you've found a plausible explanation, the incentive to keep looking disappears. The correct answer isn't always the first one that fits.
...
The insight that shifted our thinking: if a single agent struggles with large context, the answer isn't a bigger context window — it's to give each agent a _focused_ subset of the context it actually needs. A coordinator that understands the full incident can delegate to diagnostic sub-agents that each receive only the context relevant to their specific tasks. Smaller context per agent, better reasoning per agent, and the sub-agents run in parallel.
...
Dispatch every sub-agent at once. Block until the slowest one finishes. Synthesise everything together. Total time drops to the duration of the slowest sub-agent, which is much better. But the main agent is completely idle during execution. It can't react to early results, it can't tell the user what's happening, and if a user wants to inject a new hypothesis mid-run, the graph is locked inside the parallel call until everything resolves. If one sub-agent hangs, synthesis is blocked indefinitely.
...
Each sub-agent was just a tool the orchestrator could invoke. The framework handled dispatch, parallelism, and result aggregation. On paper this should have been perfect — it supports both sequential tool calls and parallel tool calls, and it gives you the orchestrator pattern for free.

In practice, neither mode fit.

In sequential mode, the orchestrator waits for each tool to return before invoking the next. That's the first execution model from above — additive latency, slow hypothesis blocks everything.

In parallel mode, the orchestrator dispatches all tools at once and blocks until every tool returns. That's the second execution model — the main agent is idle the whole time, and there's no hook between individual tool completions.

That last point was the one that killed it. There was no way, inside Deep Agents, to say "do something each time a single tool finishes, before the others have." The framework only returned control to the orchestrator once the entire batch resolved. Which meant:
...
Deep Agents are a good fit for parallel dispatch with a single aggregate step at the end and no mid-run interactivity. For our case — where incremental synthesis and user reactivity were the whole point — it was the wrong primitive.

So we started building it ourselves on raw LangGraph. With the benefit of hindsight, this was the moment to stop and re-evaluate. We didn't, and the rest of the post is largely the story is about the rabbit hole we went down.
...
### Kafka as a async medium

The sub-agent publishes a completion event to a topic. The parent agent consumes from that topic. You get durability, replay, and backpressure out of the box. You also get Kafka — brokers, consumer groups, offset management, a new operational dependency.
...
This broke the moment two sub-agents finished close together. The first completion resumed the graph and triggered main agent working loop. While the main agent was still processing, the second completion arrived and tried to resume the same graph. LangGraph either errored - there's no active interrupt to resume from - or started a fresh execution from scratch, which was worse because now we'd lost the state from the first arrival.
...
This was the spine of the whole architecture. The graph was interrupted. A result arrived in the queue. The drain loop acquired the lock, resumed the graph, let it run through synthesis until it interrupted again, then released the lock and went back to waiting on the queue. Concurrent arrivals were serialised. The graph was never resumed twice in flight.
...
But with a regular FIFO queue this had a problem. Suppose three sub-agent results were already buffered in the queue, and the user typed "also check the deployment logs from this morning." The user's message went in fourth. Worse — by the time the drain loop got to it, the graph might have already finished synthesising the original three hypotheses and reached `END`. The user's hypothesis would never spawn.
...
By this point we'd accumulated three layers of machinery, each solving a different problem. It's worth naming them clearly because they composed, and each one depended on the layer below it.

### Layer one: task identity

Every agent run got a UUID - its `task_id`. Every sub-agent carried a `parent_task_id` pointing back to whoever spawned it. The convention we settled on was that the LangGraph thread's `thread_id` was identical to the agent's `task_id`. This sounds trivial - it was the single most important convention in the system.
...
`task_id` was what was running. `parent_task_id` was what spawned it. Different identifiers, different jobs — conflating them was a reliable source of confusing bugs.

### Layer two: event queue and transport

Each agent wrapped its logic in a simple lifecycle — `working → completed | failed | canceled` — and emitted progress events into a shared event queue. Every event an agent published landed on the right channel automatically. Parent agents subscribed to the same channel to pick up sub-agent completions and resume graphs.

Alongside the PubSub broadcast, every event was also written to a durable event store indexed by `task_id`. PubSub is fire-and-forget — if you're not subscribed when a message lands, you miss it. The store covered late-joining clients catching up on an in-progress investigation. In production this would need to be a fully durable store — Back to Kafka persistence or a database — to survive pod restarts and handle crash recovery properly.
...
While we were heads-down building all of this, LangGraph was working on the same problem from the framework side.
...
We reached out to the LangGraph team to share this and ask about the roadmap. That conversation is ongoing.

So for now: we're not shipping the POC, but we're also not building on top of async sub-agents yet. The maintenance cost of owning a custom reactive runtime outweighs the benefits of shipping it today.


## Why we walked away

Here's the honest version. The POC met the requirements. The code is in the repo. And then we sat down to estimate what it would take to actually ship this — and the cost-benefit math turned hostile, fast.

Two things tipped the decision.

### The lock and queue were in-process

The POC lived entirely in one pod. The `asyncio.Queue` and `asyncio.Lock` were local to that process. Shipping this for real meant swapping them for Redis or Kafka backed equivalents for both queue and lock, and a distributed checkpointer for LangGraph state so a task could survive a pod restart. Each of those is a known-solved problem. But each is also a real piece of infrastructure to write, test, monitor, and keep correct under failure modes — pod crashes, network partitions, lost lock holders, expired TTLs. We'd be building a small distributed runtime to host a feature.

### The framework was converging on us

The async sub-agents work was the concrete form of what we'd been feeling abstractly: the gap between "what we built" and "what the framework gives you natively" was shrinking faster than we could add features on top. The requirements we could ship with the framework was meeting maybe 80% of the actual user need with maybe 20% of the implementation complexity. Less elegant. Less truly reactive. But far less infrastructure to own.

That last point is the one that's easy to miss when you're heads-down building.

When you start a research spike, the framework is a fixed thing. You measure it once, find its limits, and start building around them. But frameworks aren't fixed, especially in a space moving as fast as agent runtimes. Every month we spent on this POC was a month LangGraph was getting better at the thing we were building ourselves. The right question to keep asking — and the one we should have asked earlier — is not "can we build this?" but "if the framework ships 70% of this in six months, do we still want to be the team maintaining the other 30%?"

For us, the answer was no. The most valuable output of the spike was understanding the problem deeply enough to make that call with confidence. The conceptual work understanding the three execution models, the identity propagation pattern, the priority queue for user input, the cascading cancellation pattern — transfers cleanly to whatever runtime we end up using. The code was the dispensable part.

So we kept the learning and dropped the artifact. And the follow-up new LangGraph's Async Sub Agents were a proof that the framework could handle the majority of the use cases and acknowledged trade-offs where the current defaults fall short. Most importantly, this meant much less maintenance for us, and much more time to focus on the problems only we can solve.


## Closing thought

The thing I keep coming back to is that the most valuable output of this spike was a clearer understanding of when _not_ to build your own runtime. We started with a real problem. Single agents breaking down under the weight of context rot, instruction overload, and sequential execution. We built a working system that addressed it, and then recognised that the path to shipping it would cost more than it was worth, particularly once we saw the framework converging on the same solution.

That's not a failure. It's the version of "build vs buy" you only get to do honestly after you've actually built one of the options. You can read all the framework docs in the world and not know what you're trading off. Building the alternative, even just enough to demo it, gives you the actual feel of the trade-off. And then, ideally, you stop.

What I'd tell my past self at the start of this spike: timebox it harder, decide upfront what evidence would make you walk away, check in with the teams building the frameworks you're working on top of, and assume those frameworks are getting materially better while you're working. None of that means don't do the spike. The conceptual lessons here are real and we use them every day. It just means be honest about what you're doing — exploring a design space, not shipping a product — and let the artifact be disposable when the time comes.

That's where this one landed. A POC, some good diagrams, a working repo, a conversation with the LangGraph team, and a lot of clarity about what reactive multi-agent systems actually cost. For now, that's the right place to be.
