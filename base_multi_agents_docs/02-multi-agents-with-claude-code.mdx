...
We're using the Sub-Agents feature for this. The idea is that the orchestrator spawns sub-agents — a writer and a reviewer — and loops over the cycle until the feature is considered done.
...
For each feature, the orchestrator breaks it into tasks and writes them to `context/roadmap/features/tasks/`:
...
Since it identified two repos, it can parallelise two writer sub-agents:
...
**Next:** [Part 3 — Design doc to prototype: 7 PRs in 90 minutes](./03-design-doc-to-prototype)
