...
The full workflow runs through three slash commands:

1. `/design-doc-decomposer` — reads the design doc and produces feature files
2. `/claude-md-generator` — scrapes the repo and generates `CLAUDE.md` with validation rules
3. `/implementation-orchestrator` — implements all features, writer + reviewer loop, opens PRs

That's the whole thing. The rest of this post is what actually happened.
...
Before the implementation orchestrator runs, the sub-agents need to know how to validate their work. Every repo has its own programming language, build commands, and conventions. Rather than hardcoding these, the `claude-md-generator` skill scrapes the repo and generates a `CLAUDE.md` at the root:
